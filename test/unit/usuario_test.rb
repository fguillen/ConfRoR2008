...
class UsuarioTest < Test::Unit::TestCase
  ...
  def test_should_not_rehash_password
    ...
    assert_equal usuarios(:quentin), Usuario.authenticate('quentin2', 'test')
  end

  def test_should_authenticate_usuario
    assert_equal usuarios(:quentin), Usuario.authenticate('quentin', 'test')
  end
  ...
  protected
    def create_usuario(options = {})
      Usuario.create({ :login => 'quire', :email => 'quire@example.com', :password => 'quire', :password_confirmation => 'quire' }.merge(options))
    end
end
